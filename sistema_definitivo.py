...
"""
*fetchone() --> retorna apenas um registro; usado para confirmar se o ID existe.
*confirmacao != "s" --> exige confirmação explícita antes de deletar,
evitando exclusões acidentais.
*DELETE FROM ... WHERE id = %s --> exclui apenas o registro com o ID informado.
*rowcount não é verificado aqui pois a existência já foi checada antes com fetchone().
"""
...
def excluir_chamado():
    try:
        cursor.execute("""
            SELECT s.id, u.nome, s.descricao, s.status
            FROM chamados s
            JOIN usuarios u ON s.usuario_id = u.id
            ORDER BY s.id
        """)
