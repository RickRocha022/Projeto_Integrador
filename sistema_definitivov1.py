...
def cadastrar_usuario():
    try:
        # ── NOME ──
        nome = input("\nNome: ").strip()    #strip() --> remove espaços em branco no começo e no final
        if not nome:
        ...
        try:
            float(email)
            print("Email não pode ser um número!")
            return
        except ValueError:
            pass
...
def listar_usuarios():
    try:
        ...
        usuarios = cursor.fetchall()    #fetchall() --> pega todos os resultados retornados pela última consulta feita
...
def abrir_chamado():
    try:
        ...
        usuario_id = input("Informe o ID do usuário: ").strip()
        ...
        categoria_id = input("Informe o ID da categoria: ").strip()
        descricao    = input("Descrição do problema: ").strip()

        if not descricao:
            print("Descrição não pode estar em branco!")
            return

        print("\nUrgência (1 = baixa  →  5 = crítica)")
        urgencia = int(input("Urgência [1-5]: ").strip())

        print("Impacto  (1 = baixo  →  5 = alto)")
        impacto  = int(input("Impacto  [1-5]: ").strip())

        if not (1 <= urgencia <= 5) or not (1 <= impacto <= 5):
        ...
        print("Valor inválido! Informe apenas números onde solicitado.")
    except Exception as e:
